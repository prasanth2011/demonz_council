...
namespace Drupal\flysystem_objective\Plugin\ImcePlugin;
...
class Info extends ImcePluginBase {
  ...
  public function permissionInfo() {
    return [
      'resize_images' => $this->t('Meta information for objective files'),
    ];
  }
  ...
  public function buildPage(array &$page, ImceFM $fm) {
    ...
    if ($fm->hasPermission('resize_images')) {
      //$page['#attached']['library'][] = 'objective/drupal.imce.info';
    }
  }
  ...
  public function opInfo(ImceFM $fm) {
    ...
    if ($this->validateInfo($fm, $items, $width, $height, $copy)) {
   ...
   */
  public function validateInfo(ImceFM $fm, array $items, $width, $height, $copy) {
  ...
  /**
   * Infos a list of imce items and returns succeeded ones.
   */
